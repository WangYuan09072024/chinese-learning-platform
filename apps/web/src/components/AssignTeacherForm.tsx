...
import { assignTeacher } from '@/actions/course-teachers';

export function AssignTeacherForm({ courseSlug, courseId }: { courseSlug: string; courseId: string }) {
  const action = assignTeacher.bind(null, courseSlug, courseId);
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-2">
      ...
      <button type="submit" disabled={pending} className="rounded-md bg-zinc-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-50 dark:bg-zinc-100 dark:text-zinc-900">
        {pending ? 'Đang gán...' : 'Gán giáo viên'}
      </button>
      {state.error && <p className="w-full text-sm text-red-500">{state.error}</p>}
      {state.success && <p className="w-full text-sm text-green-600">Gán giáo viên thành công!</p>}
    </form>
  );
}
